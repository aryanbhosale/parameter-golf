"""ULTIMATE: slope0.75 + VE128 + mHC + no-SWA + QAT50% + Muon-TTT + entropy-adaptive + LZMA. Target: #1."""
...
try:
    import zstandard
    def _compress_zstd(data: bytes) -> bytes: return zstandard.ZstdCompressor(level=22).compress(data)
    def _decompress_zstd(data: bytes) -> bytes: return zstandard.ZstdDecompressor().decompress(data)
except ImportError:
    def _compress_zstd(data: bytes) -> bytes: return zlib.compress(data, level=9)
    def _decompress_zstd(data: bytes) -> bytes: return zlib.decompress(data)
def _compress_lzma(data: bytes) -> bytes: return _lzma_mod.compress(data, preset=7)
def _decompress_lzma(data: bytes) -> bytes: return _lzma_mod.decompress(data)
def _compress(data: bytes) -> bytes: return _compress_lzma(data) if _USE_LZMA else _compress_zstd(data)
def _decompress(data: bytes) -> bytes: return _decompress_lzma(data) if _USE_LZMA else _decompress_zstd(data)
...
class Hyperparameters:
    ...
    scalar_lr = float(os.environ.get("SCALAR_LR", 0.027))
    ...
    use_gated_attention = bool(int(os.environ.get("USE_GATED_ATTENTION", "0")))
    ...
    qat_time_frac = float(os.environ.get("QAT_TIME_FRAC", 0.50))
        ...
        if freeze:
            ...
            for bi in range(num_blocks):
                if f"blocks.{bi}." in name:
                    ...
                    lr_mul = 0.5 + 1.5 * depth_frac
                    break
            if "mlp_down" in name or "proj" in name:
                lr_mul *= 1.5
            ttt_param_groups.append({'params': [p], 'lr': args.ttt_lr * lr_mul})
            ttt_params_all.append(p)
            ...
            if chunk_seqs > 0:
                ...
                if not use_muon_ttt and optimizer is not None:
                    for pg in optimizer.param_groups:
                        pg['lr'] = pg.get('_base_lr', pg['lr']) * (0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1))))
                    if ci == 0:
                        for pg in optimizer.param_groups:
                            pg['_base_lr'] = pg['lr'] / (0.5 * (1.0 + math.cos(0.0)))
                ...
                if args.ttt_entropy_adapt:
                    if chunk_nll > args.ttt_entropy_high:
                        effective_epochs = args.ttt_epochs + 1
                    elif chunk_nll < args.ttt_entropy_low:
                        effective_epochs = max(args.ttt_epochs - 1, 1)
                # Wall-clock guard
                elapsed_now = time.perf_counter() - t0
                if elapsed_now > 570 * 0.85:
                ...
                for _ep in range(effective_epochs):
                    for bs in range(0, my_chunk_seqs, batch_seqs):
                        ...
                        torch.nn.utils.clip_grad_norm_(ttt_params_all, args.ttt_grad_clip)
                        if not use_muon_ttt and optimizer is not None:
                            optimizer.step()
                        else:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                w_q = (torch.clamp(torch.round(w32 / scale[:, None]), -INT6_RANGE, INT6_RANGE) * scale[:, None]).to(x.dtype)
